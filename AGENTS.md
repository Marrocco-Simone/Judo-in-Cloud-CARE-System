...
- `scoreboard-overlay.js` — Draws the Shiai second monitor (waiting, pre-fight, match, end screens) on a canvas: on the stream frame at the current time, and in the player picture-in-picture at the recording time of the frame on screen (`getDisplayedTimestamp` in `camera.js`), so a rewind shows the old score. Polls the live-keeper state once per second from page load and keeps one snapshot per state change in RAM. It reconstructs the match and osaekomi clocks like the Shiai live tatami page, timing each state with its `age_ms` from the live-keeper, so the clocks of the PCs do not matter. It mirrors `shiai/frontend/components/pages/live-tatami` and `match-timer`; the two logos are copied from `shiai/frontend/public`.
